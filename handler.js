...
module.exports.send = (event, context, callback) => {
  console.log(`https://sqs.${process.env.REGION}.amazonaws.com/${awsAccountId}/RadloopSNSExpiredRequestEvents${sqsPostfix}`);

  sqs.sendMessage({
    DelaySeconds: 0,
    MessageBody: JSON.stringify(event),
    QueueUrl: `https://sqs.${process.env.REGION}.amazonaws.com/${awsAccountId}/RadloopSNSExpiredRequestEvents${sqsPostfix}`,
  }, function (err, data) {
    if (err) {
